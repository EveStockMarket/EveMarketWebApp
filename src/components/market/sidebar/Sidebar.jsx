...
import SidebarFilters from "./filters_bar/SidebarFilters";
import './Sidebar.css';
...
function DisplayIcon({ hasChildren, isOpen, node }) {
  ...
  const getNodeIcon = () => {
    const iconPath = node.iconId ? `https://api.evestockmarket.pl/icon_32/${node.iconId}` : '/assets/main_icons/0_32.webp';
    return (
      <LazyImage
        src={iconPath}
        alt={`${node.name} icon`}
